-- reaper-edit-tools
-- Set Source IN: Places a source IN marker at current cursor/play position
-- Author: Ben Kazez
...
function main()
    ...
    local marker_name = common.make_marker_name(track_num, common.SOURCE_IN)
    
    if cur_pos ~= -1 then
        common.clear_existing_markers_of_type(common.SOURCE_IN)
        AddProjectMarker2(0, false, cur_pos, 0, marker_name, -1, common.SOURCE_COLOR)
        UpdateArrange()
    end
    
    Undo_EndBlock("Set Source IN", 0)
end
